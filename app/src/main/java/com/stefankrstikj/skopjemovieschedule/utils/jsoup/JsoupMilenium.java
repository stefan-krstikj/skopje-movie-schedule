...
public class JsoupMilenium extends Jsoup {
    ...
    public Pair<List<Movie>, List<MovieSchedule>> getPairMovieAndSchedule() throws IOException {
        ...
        try{
            Document doc = org.jsoup.Jsoup.connect(URLList.URLMilenium).get();
    ...
    public void getDetailedInfo(Movie movie, List<MovieSchedule> movieSchedules) throws IOException {
        ...
        Connection.Response response = org.jsoup.Jsoup.connect(movie.getDetailsURL()).timeout(20 * 1000)
                .execute();
        final Document doc = org.jsoup.Jsoup.parse(response.body());
